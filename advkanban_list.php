...
/**
 *   	\file       advkanban_list.php
 *		\ingroup    advancedkanban
 *		\brief      List page for advkanban
 */
...
$contextpage = GETPOST('contextpage', 'aZ') ? GETPOST('contextpage', 'aZ') : 'advkanbanlist'; // To manage different context of search
...
$hookmanager->initHooks(array('advkanbanlist')); // Note that conf->hooks_modules contains array
...
if ($enablepermissioncheck) {
	...
	$permissiontodelete = $user->hasRight('advancedkanban', 'advkanban', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'AdvKanban';
	$objectlabel = 'AdvKanban';
...
//$help_url = "EN:Module_AdvKanban|FR:Module_AdvKanban_FR|ES:Módulo_AdvKanban";
$help_url = '';
$title = $langs->trans("AdvKanbans");
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalString('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/advancedkanban/advkanban_card.php', 1).'?id='.$id);
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/advancedkanban/advkanban_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendAdvKanbanRef";
$modelmail = "advkanban";
$objecttmp = new AdvKanban($db);
