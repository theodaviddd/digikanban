...
/**
 * \file    advancedkanban/admin/setup.php
 * \ingroup advancedkanban
 * \brief   AdvancedKanban setup page.
 */
...
require_once '../lib/advancedkanban.lib.php';
//require_once "../class/myclass.class.php";

// Translations
$langs->loadLangs(array("admin", "advancedkanban@advancedkanban"));

// Initialize technical object to manage hooks of page. Note that conf->hooks_modules contains array of hook context
$hookmanager->initHooks(array('advancedkanbansetup', 'globalsetup'));
...
$item = $formSetup->newItem('ADVKANBAN_DEFAULT_KANBAN_CONTACT_CODE');

$sql = "SELECT code, libelle FROM  ".MAIN_DB_PREFIX."c_type_contact WHERE active=1 AND element='advancedkanban_advkanbancard' AND source='internal' LIMIT 10";
$contactsType = $db->getRows($sql);
$TSelect = array();
if($contactsType){
	foreach ($contactsType as $contactType){
		$TSelect[$contactType->code] = $langs->trans($contactType->libelle);
		if(getDolGlobalString('ADVKANBAN_DEFAULT_KANBAN_CONTACT_CODE') == $contactType->code){
			$item->fieldOutputOverride = $TSelect[$contactType->code];
		}
	}
}
$item->fieldInputOverride = $form->selectArray('ADVKANBAN_DEFAULT_KANBAN_CONTACT_CODE', $TSelect, getDolGlobalString('ADVKANBAN_DEFAULT_KANBAN_CONTACT_CODE'));



$formSetup->newItem('ADVKANBAN_SETUPTITLE_KANBAN')->setAsTitle();



$item = $formSetup->newItem('ADVKANBAN_KANBAN_DISPLAY_STATUS_MODE');
$TField = array(
...
$item = $formSetup->newItem('ADVKANBAN_UNSPLASH_API_KEY');
$item->helpText = $langs->trans('ADVKANBAN_UNSPLASH_API_KEY_HELP');
...
} elseif ($action == 'specimen')
{
	...
	{
		$file = dol_buildpath($reldir."core/modules/advancedkanban/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file))
		{
			$filefound = 1;
...
elseif ($action == 'setmod') {
	...
	if (!empty($tmpobjectkey)) {
		$constforval = 'ADVANCEDKANBAN_'.strtoupper($tmpobjectkey)."_ADDON";
...
elseif ($action == 'set') {
	...
	if ($ret > 0) {
		$tmpobjectkey = GETPOST('object');
		if (!empty($tmpobjectkey)) {
			$constforval = 'ADVANCEDKANBAN_'.strtoupper($tmpobjectkey).'_ADDON_PDF';
...
$head = advancedkanbanAdminPrepareHead();
print dol_get_fiche_head($head, 'settings', 'Setup', -1, "advancedkanban@advancedkanban");

// Setup page goes here
echo '<span class="opacitymedium">'.$langs->trans("AdvancedKanbanSetupPage").'</span><br><br>';
...
$moduledir = 'advancedkanban';
$myTmpObjects = array();
$myTmpObjects['AdvKanban'] = array('includerefgeneration'=>1, 'includedocgeneration'=>0);
$myTmpObjects['AdvKanbanCard'] = array('includerefgeneration'=>0, 'includedocgeneration'=>0);
					...
					while (($file = readdir($handle)) !== false)
					{
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php')
						{
							...
							if ($module->isEnabled())
							{
								...
								$constforvar = 'ADVANCEDKANBAN_'.strtoupper($myTmpObjectKey).'_ADDON';
